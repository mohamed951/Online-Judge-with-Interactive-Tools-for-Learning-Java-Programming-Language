	...
	<script src="<?php echo SITE_URL ?>/js/customblock.js"></script>	  
                    ...
                    $valid=true;
                     if(isset($_SESSION['loggedin'])){
                          if($_SESSION['team']['status'] != 'Admin'){
                             ...
                             AND ( contest.groupname = groups.groupname OR contest.groupname = 'public' ) ) ) AND contest.code = '".$result['pgroup']."'";
                             $contest = DB::findOneFromQuery($query);
                             if(!$contest)$valid=false;
                          } 
                     }
                         else $valid=false;
                              
                    if(!$valid){
                        echo errorMessageHTML("<b>Can't Access This Problem</b>");
                        exit();
                    }
    ...
    //----------------------------------------------------------------------------------------------------------------------------
         ...
         <div class="row">
             ...
             <form id='form' action='<?php echo SITE_URL; ?>/process.php' method='post' enctype='multipart/form-data'>
                     <div class="col-md-6" style="overflow-x:auto;height:500px;"  id="blocklyDiv" ></div>
                     <textarea class="col-md-6" id="sub1" name="sub1" ></textarea>
                    ...
                    <input type="hidden" value="Java" name="lang"/>
                ...
                <div class='row'>
                    ...
                    <div class='panel2 panel-default'>
                        <div class='panel-body text-center'>
                            <xml style="display:none" id="default">
                                <block type="main_class">
                                <statement name="NAME">
                                    <block type="main_method"></block>
                                </statement>
                            </block>
                            </xml>
                  ...
                  <script>
                        var myCodeMirror,output = '';
                             $(document).ready(function() {
                                myCodeMirror = CodeMirror.fromTextArea(document.getElementById('sub1'), {'matchBrackets': true, 'autoCloseBrackets': true, 'lineWrapping': true, 'mode': 'text/x-java', 'lineNumbers': true,
...
//----------------------------------------------------------------------------------------------------------------------------
                    ...
                    echo "<span class='btn-group' style='float:right;'></span></div>";
                     echo "<h1>Find errors in code by using click and you have 3 times error</h1><br/>
                      <div class='row'>
                    <div class='col-md-12'>
                    <i class='glyphicon glyphicon-heart' id='heart1' style='font-size:36px;color:red'></i>
                <i class='glyphicon glyphicon-heart' id='heart2' style='font-size:36px;color:red'></i>
                <i class='glyphicon glyphicon-heart' id='heart3' style='font-size:36px;color:red'></i>
...
<?php
        ...
        if (isset($_SESSION['loggedin']) && $_SESSION['team']['status'] == "Admin") {
            $query = "select pid, name, code, status, pgroup, type, score, solved, total,pcat from problems where contest='practice' order by pgroup asc";
        } else {
            $query = "select pid, name, code, status, pgroup, type, score, solved, total, pcat from problems where status != 'Deleted' and contest='practice' order by pgroup asc";
        }
        $res = DB::findAllFromQuery($query);
        $lastgroup = "";
        $counter=0;
        ...
        foreach ($res as $row) {
            ...
            echo "<tr ".((isset($solved[$row['pid']]))?("class='success'"):(""))."><td><a href='" . SITE_URL . "/problems/$row[code]'>$row[name]</a></td><td class='tabletaghidden' >$row[type]</td><td><a href='" . SITE_URL . "/submit/$row[code]'>$row[code]</a></td><td><a href='" . SITE_URL . "/status/$row[code]'>$row[solved]/$row[total]</a></td><td><span class='badge'>$row[score] pt</span></td><td>" . (isset($editorial[$row['pid']])?("<a href='" . SITE_URL . "/editorial/$row[code]'>Link</a>"):(($_SESSION['team']['status'] == 'Admin')?"<a href='". SITE_URL . "/admineditorial/$row[code]'>Add</a>":"None")) . "</td></tr>";
             ...
             echo "<tr ".((isset($solved[$row['pid']]))?("class='success'"):(""))."><td><a href='" . SITE_URL . "/problems/$row[code]'>$row[name]</a></td><td class='tabletaghidden' >$row[type]</td><td>$row[code]</td><td>$row[solved]/$row[total]</td><td><span class='badge'>$row[score] pt</span></td><td>" . (isset($editorial[$row['pid']])?("<a href='" . SITE_URL . "/editorial/$row[code]'>Link</a>"):(($_SESSION['team']['status'] == 'Admin')?"<a href='". SITE_URL . "/admineditorial/$row[code]'>Add</a>":"None")) . "</td></tr>";
