...
<div class="text-center page-header"><?php if(isset($_SESSION['loggedin']) && $_SESSION['team']['status'] == "Admin"){ ?><?php } ?><h1>Rankings - <?php echo $contest['name'] ?></h1></div>
    ...
    foreach ($rank as $val) {
        ...
        $probCells = "";
        $solvedProb= preg_split("/,/", $val['prob']);
        foreach($pidToProbCode as $pid=>$code){
            ...
            */
            $probCells .= "<td style='text-align:center'>". (in_array($pid, $solvedProb)?"<span class='glyphicon glyphicon-ok' style='color: green'></span> ":"-") ."</td>";
