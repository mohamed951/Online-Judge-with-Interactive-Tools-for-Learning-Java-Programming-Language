...
if ($judge['value'] != "Lockdown" || (isset($_SESSION['loggedin']) && $_SESSION['team']['status'] == 'Admin')) {
    ...
    $sql = "select * from teams where status='Normal'";
    if (isset($tab)){
         $sql = "select * from teams,teamgroups,groups where status='Normal' and teams.tid=teamgroups.tid and teamgroups.gid=groups.gid and groups.gid=$groupval[$tab]";
    }
    $sql .= " ORDER BY score DESC, penalty ASC";
    $result = DB::findAllWithCount($sql,"",$page, 20);
    $data = $result['data'];
    $i = 20 * ($page - 1) + 1;
   /* $select = "SELECT * ";
