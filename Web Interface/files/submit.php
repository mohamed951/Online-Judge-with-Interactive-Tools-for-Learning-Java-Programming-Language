    ...
    if (!isset($_SESSION['loggedin'])) {
        ...
        if (isset($_GET['code']) || isset($_GET['edit'])) {
            ...
            if($result['contest'] == 'contest'){
                    $valid=true;
                     if(isset($_SESSION['loggedin'])){
                          if($_SESSION['team']['status'] != 'Admin'){
                             $query2="SELECT * FROM contest, groups, teamgroups WHERE ( contest.groupname = 'public' OR ( groups.gid = teamgroups.gid AND teamgroups.tid =".$_SESSION['team']['id']."
                             AND ( contest.groupname = groups.groupname OR contest.groupname = 'public' ) ) ) AND contest.code = '".$result['pgroup']."'";
                             $contest = DB::findOneFromQuery($query2);
                    ...
                    if(!$valid){
                        echo errorMessageHTML("<b>Can't Submit a Solution to This Problem</b>");
                        exit();
                    }
            }
