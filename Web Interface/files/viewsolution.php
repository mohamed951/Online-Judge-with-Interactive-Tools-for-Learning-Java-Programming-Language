...
if ($judge['value'] != "Lockdown" || (isset($_SESSION['loggedin']) && $_SESSION['team']['status'] == 'Admin')) {
    if (isset($_GET['code'])) {
        ...
        if ($res) {
            ...
            $sql = "select input,output from testcase where pid=".$res[pid];
            $InputsAndOutput=DB::findAllFromQuery($sql);
            $input=""; $output="";
            foreach($InputsAndOutput as $row){ // iterate 
                    $input.=$row['input'];
                    $output.=$row['output'];
                }
            $prob[input]=$input;
...
//----------------------------------------mena-------------------------------------------------------------------------------
                $sql2 = "select pcat from problems where pid=".$res[pid];
            $check=DB::findOneFromQuery($sql2);
                if($check["pcat"]=="Text"||$check["pcat"]=="blockly"){
