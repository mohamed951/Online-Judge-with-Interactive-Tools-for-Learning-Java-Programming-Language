...
function down() { 
 ...
 function play(playtimes=0){
     if(playtimes==1)
     	reset1(0);
        if(parseInt(elem.style.left)==300 && parseInt(elem.style.top)==195 && i>=arr.length){
	...
	if(i<arr.length){        
     if(arr[i]=="forward" && parseInt(elem.style.top)==195 && parseInt(elem.style.left)<300 ) 
			    left();
            else {
            elem.innerHTML='<img src="thumbs_explosion-animation.gif" width="100" hight="100" id="elementimg"/>';
                SubmitGame(0);
            }
		}
		else {
